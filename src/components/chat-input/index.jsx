...
const ChatInput = ({messageId}) => {
    ...
    const {dispatch, chatMessages, editMessage, chatDraft, chatPopupAnswers,formData, chatIndex} = useContext()
    const [chatText, setChatText] = useState(chatDraft)
    ...
    useEffect(() => {
        ...
        if (chatText) {
            textareaEl.current.textContent = chatText;
        }
    }, [editMessage, chatText]);
    ...
    function changeMessage() {
        if (chatText) {
            ...
            dispatch({type: actions.EDIT_MESSAGE, payload: false})
            textareaEl.current.textContent = "";
            setChatText("");
        }
    }

    function cancelChangeMessage() {
        dispatch({type: actions.EDIT_MESSAGE, payload: false})
        textareaEl.current.textContent = "";
        setChatText("");
    }

    function createMessage(customText, is = false, itemId) {
        if (chatText || customText) {
            dispatch({
                type: actions.ADD_MESSAGE, payload: {
                    title: customText ? customText : chatText,
    ...
    async function clickCreateMessage() {
        await createMessage(chatText)
        textareaEl.current.textContent = "";
        setChatText("");
    ...
    async function createQuestion() {
        // setTimeout(() => {
        await createMessage(`${formData ? formData.items.length - chatIndex-1 : 1} more questions to go`,true)
        await createMessage(`Question ${chatIndex+1} out of ${formData ? formData.items.length : 1}`,true)

        if (chatIndex+1 === Math.round(formData.items.length/2)) {
            await createMessage(`Halfway through`,true)
        }

        await createMessage(formData.items[chatIndex].title, true, formData.items[chatIndex].itemId)
        if (formData.items[chatIndex].description) {
            await createMessage(formData.items[chatIndex].description, true)
        }
    ...
    useEffect(() => {
        if (formData) {
            if (chatMessages.find(message => message.title === '/signin')&&!chatMessages.find(message => message.title === formData.items[chatIndex].title)) {
                createQuestion()
            }
        }
    }, [chatIndex])
    ...
    return (
        <div className="chat-input">
            {
                chatMessages.find(message => message.title === '/signin') ?
                        ...
                        <div className="rows-wrapper chat-input-wrapper">
                            <svg className="bubble-tail" width="9" height="20" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <g fill="none" fillRule="evenodd">
                                    <path d="M3 17h6V0c-.193 2.84-.876 5.767-2.05 8.782-.904 2.325-2.446 4.485-4.625 6.48A1 1 0 003 17z" fill="#000" filter="url(#a)"/>
                                    <path d="M3 17h6V0c-.193 2.84-.876 5.767-2.05 8.782-.904 2.325-2.446 4.485-4.625 6.48A1 1 0 003 17z" fill="#FFF" className="corner"/>
                                </g>
                            </svg>

                            <div className="new-message-wrapper">
                                <Button
                                    className={cx('btn-icon silver input', {
                                        'cancel': !editMessage,
                                        'emoji': editMessage
                                    })}
                                    onClick={editMessage ? cancelChangeMessage : ''}
                                >
                                    ...
                                    <svg className="cancel" xmlns="http://www.w3.org/2000/svg" version="1.0"
                                    ...
                                    <svg className="emoji" xmlns="http://www.w3.org/2000/svg" version="1.0" width="344.000000pt" height="344.000000pt" viewBox="0 0 344.000000 344.000000" preserveAspectRatio="xMidYMid meet">
                                ...
                                <div className="input-message-container">
                                    <div
                                        ...
                                        onInput={(event) => {
                                            ...
                                            setChatText(event.target.textContent)
                                        ...
                                        onKeyPress={event => {
                                            if (event.key === 'Enter') {
                                                editMessage ? changeMessage() : clickCreateMessage()
                                            }
                                        }}
                                    />
                                </div>
                                ...
                                <Button
                                    ...
                                    onClick={() => dispatch({type: actions.CHAT_POPUP_ANSWERS, payload: !chatPopupAnswers})}
                                >⌘</Button>
                                ...
                                <Button
                                    className={cx('btn-icon blue input', {
                                        'send': editMessage ? false : true,
                                        'check': editMessage,
                                        'microphone': editMessage ? false : !chatText
                                    })}
                                    onClick={editMessage ? changeMessage : clickCreateMessage}
                                >
